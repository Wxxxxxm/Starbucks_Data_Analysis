...
def find_similar(df, attr, query_str, str_list):
    ...
    str_with_sim = str_with_sim.sort_values(by=['similarity'], ascending=False)
    #print str_with_sim
    str_temp = str_with_sim.reset_index(drop=True)

    a=0
    for a in range(len(str_temp)):
        if(str_temp['similarity'][a] !=str_temp['similarity'][a+1] ):
            break

    res = str_temp[0: a+1].pop('str').tolist()
...
if __name__ == "__main__":
    ...
    df_find = find_same(starbucks_df, 'City', '厂州市')
    df_find = preprocessing(df_find)
    df_temp = topk(df_find, 113.23, 23.40, 5)
    print(df_temp)
